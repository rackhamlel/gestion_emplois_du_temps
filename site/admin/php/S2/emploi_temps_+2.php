...
<nav id="nav-1">
        ...
        <a class="link-1" href="emploi_temps_+2.php">S+2</a>
    ...
    <table>
        ...
        <?php
        require_once 'utilisateur_travail+2.php';
        ...
        echo "</table>";
            if ($_SESSION['admin']==1){
                ...
                function getStartAndEndOfWeekAfterNext($date) {
                    ...
                    $mondayAfterNextWeek = strtotime('monday +2 weeks', strtotime($date));
                    // Calculer le dimanche de la semaine après la semaine prochaine
                    $sundayAfterNextWeek = strtotime('sunday +2 weeks', strtotime($date));
                    return [date('Y-m-d 00:00:00', $mondayAfterNextWeek), date('Y-m-d 23:59:59', $sundayAfterNextWeek)];
                }
                
                list($startOfWeekAfterNext, $endOfWeekAfterNext) = getStartAndEndOfWeekAfterNext(date('Y-m-d'));
                

                // Préparer la requête de sélection des notes de la semaine après la semaine prochaine
                ...
                WHERE DateNote BETWEEN :startOfWeekAfterNext AND :endOfWeekAfterNext";
                $stmt = $connexion->prepare($sql);

                // Lier les paramètres
                $stmt->bindParam(':startOfWeekAfterNext', $startOfWeekAfterNext);
                $stmt->bindParam(':endOfWeekAfterNext', $endOfWeekAfterNext);
    ...
    <script src="/js/emploi_2.js"></script>
</body>
</html>
