...
<nav id="nav-1">
    ...
    try {
        ...
        $requete = "SELECT Utilisateur.Nom,Utilisateur.id_utilisateur, SUM(TIMESTAMPDIFF(HOUR, EmploiDuTemps.DateHeureEmbauche, EmploiDuTemps.DateHeureDebauche)) AS NombreHeures
                FROM Utilisateur
                LEFT JOIN EmploiDuTemps ON Utilisateur.id_utilisateur = EmploiDuTemps.id_utilisateur
        ...
        foreach ($resultats as $row) {
            $afficherUtilisateur .= "<li class='user-list-item'><button class='editBtn' data-id='" . htmlspecialchars($row["id_utilisateur"]) . "'>" . htmlspecialchars($row["Nom"])  . "</button><p>Heures affectées : ".htmlspecialchars($row["NombreHeures"])."</p></li>";
